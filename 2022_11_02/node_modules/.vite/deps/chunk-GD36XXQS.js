...
var createStoreForExport = (initialValues) => {
  ...
  const asyncGet = (atom2) => new Promise((resolve, reject) => {
    const atomState = store[READ_ATOM](atom2);
    if ("e" in atomState) {
      reject(atomState.e);
    } else if ("p" in atomState) {
      resolve(atomState.p.then(() => asyncGet(atom2)));
    } else {
      resolve(atomState.v);
    }
  });
  const set = (atom2, update) => store[WRITE_ATOM](atom2, update);
  const sub = (atom2, callback) => store[SUBSCRIBE_ATOM](atom2, callback);
  return {
    get,
    asyncGet,
    set,
...
//# sourceMappingURL=chunk-GD36XXQS.js.map
